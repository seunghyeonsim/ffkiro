...
public:
    FlipperAttitudeControl() : Node("flipper_attitude_control")
    {
        ...
        // 파라미터
        Kp_omega_      = this->declare_parameter<double>("Kp_omega", 1.0);
        tau_rho_       = this->declare_parameter<double>("tau_rho", 2.0);
        rho_dot_max_   = this->declare_parameter<double>("rho_dot_max_deg", 20.0);
        w_task_        = this->declare_parameter<double>("w_task", 1.0);
        w_rho_         = this->declare_parameter<double>("w_rho", 1e-3);
        w_damp_base_   = this->declare_parameter<double>("w_damp_base", 1e-4);
...
private:
    ...
    {
        // 토픽 수신 대기 상태 출력
        if (!imu_received_ || !joint_received_ || !flipper_received_) {
            RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                "⏳ Waiting for topics... (IMU:%s, Joint:%s, Flipper:%s)",
                imu_received_ ? "✅" : "❌",
                joint_received_ ? "✅" : "❌",
                flipper_received_ ? "✅" : "❌");
        }
    ...
    void compute_rho_dot()
    {
        ...
        Eigen::Vector2d omega_xy = Kp_omega_ * omega_e.head<2>();
        ...
        const double lo = params_.RHO_LO;
        const double hi = 0.0;
    ...
    std::string joint_fl_name_, joint_fr_name_, joint_rl_name_, joint_rr_name_;

    bool has_flipper_pos_=false;
    double last_fl_=0.0, last_fr_=0.0, last_rl_=0.0, last_rr_=0.0;

    double Kp_omega_, tau_rho_, rho_dot_max_, w_task_, w_rho_, w_damp_base_;
    ...
    void on_joint_state(const sensor_msgs::msg::JointState::SharedPtr msg)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "✅ JointState received (size=%zu)", msg->name.size());
        ...
        Eigen::Vector4d rho = control_->getRho();
        if (i_fl >= 0 && i_fl < (int)msg->position.size()) rho(0) = msg->position[i_fl];
        if (i_fr >= 0 && i_fr < (int)msg->position.size()) rho(1) = msg->position[i_fr];
        if (i_rl >= 0 && i_rl < (int)msg->position.size()) rho(2) = msg->position[i_rl];
        if (i_rr >= 0 && i_rr < (int)msg->position.size()) rho(3) = msg->position[i_rr];
        control_->setRho(rho);
    }
};
